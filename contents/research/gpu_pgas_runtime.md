...
一方、PGASは「一方が他方のメモリに一方的に書き込める」というAPIであり、同期を必要としない分GPUプログラミングとも相性が良く、GPU側からCPUを介さず直接 PUT/GET が行えるNVSHMEMのようなPGASライブラリも登場しています。

例えばマルチGPUシステムの典型的な利用例である大規模な深層学習においては、モデルのレイヤ間の通信やテンソルの分割計算などの比較的固定的な通信パターンにおいてはMPI-likeな通信が利用される一方で、 Mixture-of-Experts (MoE) などのより複雑なモデルの登場に伴い、一部の通信に PGAS によるより柔軟な通信パターンが用いられ始めています [^3] 。
